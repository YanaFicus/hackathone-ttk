      ...
      <div className={`p-4 rounded-xl mb-6 flex items-center gap-4 ${isRecording ? 'bg-red-50 border border-red-200' : 'bg-pink-50'}`}>
        ...
        <div className="flex-1">
          <h3 className="font-medium text-gray-900">
            {isRecording ? '🎙️ Запись голоса в эфир' : '🎤 Запись с микрофона'}
          </h3>
          <p className="text-sm text-gray-600">
            {isRecording 
              ? 'Ваш голос транслируется в прямом эфире' 
              : isBroadcasting 
                ? 'Нажмите "Записать" чтобы начать голосовую трансляцию' 
                : 'Начните вещание чтобы использовать голос в эфире'}
          </p>
        </div>
        <button
      ...
      {isRecording && (
        <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
          <p className="text-xs text-green-700 flex items-center gap-2">
            ...
            🎙️ Ваш голос транслируется в прямом эфире
          </p>
        </div>
      )}
    </div>
